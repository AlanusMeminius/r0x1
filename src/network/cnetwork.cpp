...
QString GetRpcUri() {
  return QString("http://localhost:%1/jsonrpc").arg(6800);
}

}// namespace

AriaClient::AriaClient(QObject *parent)
    : QThread(parent), m_syncNetworkAccessManager(new QNetworkAccessManager(this)), m_asyncNetworkAccessManager(nullptr) {
  start();
}
